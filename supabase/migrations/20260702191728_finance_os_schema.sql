...
CREATE TABLE accounts (
  ...
  name TEXT NOT NULL,
  type TEXT NOT NULL CHECK (type IN ('savings', 'current', 'credit', 'cash', 'wallet', 'upi')),
...
-- 5. Budget Rules (The Core Engine)
CREATE TABLE budget_rules (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID REFERENCES profiles(id) ON DELETE CASCADE NOT NULL,
  category_id UUID REFERENCES categories(id) ON DELETE SET NULL,
  name TEXT NOT NULL,
  type TEXT NOT NULL CHECK (type IN ('daily', 'weekly', 'monthly', 'fixed')),
  amount DECIMAL(12,2) NOT NULL,
  day_multiplier DECIMAL(5,2) DEFAULT 1.0,
  is_active BOOLEAN DEFAULT true,
  created_at TIMESTAMPTZ DEFAULT NOW(),
  updated_at TIMESTAMPTZ DEFAULT NOW()
);
...
-- 6. Recurring Expenses (Auto-import into Planner)
...
CREATE TABLE investments (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID REFERENCES profiles(id) ON DELETE CASCADE NOT NULL,
  type TEXT NOT NULL CHECK (type IN ('sip', 'stock', 'mutual_fund', 'fd', 'ppf', 'gold', 'crypto', 'other')),
  name TEXT NOT NULL,
  amount_invested DECIMAL(12,2) DEFAULT 0,
  current_value DECIMAL(12,2) DEFAULT 0,
  sip_amount DECIMAL(12,2),
  sip_day INT CHECK (sip_day BETWEEN 1 AND 31),
  provider TEXT,
  notes TEXT,
  created_at TIMESTAMPTZ DEFAULT NOW(),
  updated_at TIMESTAMPTZ DEFAULT NOW()
...
-- 9. Monthly Snapshots (For Reports & Analytics)
...
CREATE INDEX idx_snapshots_user ON monthly_snapshots(user_id);
...
CREATE POLICY "Users manage own budget rules"
